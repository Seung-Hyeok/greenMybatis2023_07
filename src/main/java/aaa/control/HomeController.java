...
public class HomeController {
	...
	String control(Model mm) {
		...
		maps.put("barrey",new  HandPhone("딸기폰",100));
	...
	String joinForm(Member mem) { // Member mem 가 있어야 joinForm을 get으로 열었을 때 멤버요소에 접근가능하여 에러가 나지 않는다
		// 반대로 Member mem이 없다면 에러발생
	...
	String joinComplete(@Valid Member mem, BindingResult bRes) { // @Valid --> spring 에서 이 객체(Member)에 대한 멤버변수의 에러 점검
		System.out.println("/basic/joinComplete 실행");
		
		// 에러가 있다면 form 페이지로 view 변경
		if(joinChk.hasErros(mem, bRes)) {
			return "basic/joinForm"; // view 페이지 진입이다, url변경이 아님
		}
